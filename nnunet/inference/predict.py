...
def preprocess_save_to_queue(preprocess_fn, q, list_of_lists, output_files, segs_from_prev_stage, classes,
    ...
    for i, l in enumerate(list_of_lists):
        # print("process in line 42")
        try:
            # print("process in line 44")
            output_file = output_files[i]
            print("preprocessing", output_file)
            d, _, dct = preprocess_fn(l)
            # print(output_file, dct)
            # print("process in line 49")
            if segs_from_prev_stage[i] is not None:
                ...
                # print("process in line 62")
                seg_prev = seg_prev.transpose(transpose_forward)
                # print("process in line 64")
                seg_reshaped = resize_segmentation(seg_prev, d.shape[1:], order=1)
                # print("process in line 66")
                seg_reshaped = to_one_hot(seg_reshaped, classes)
                # print("process in line 68")
                d = np.vstack((d, seg_reshaped)).astype(np.float32)
                # print("process in line 70")
            ...
            # print(d.shape)
            # print("process in line 79")
            # print("np.prod {}".format(np.prod(d.shape)))
#             if np.prod(d.shape) > (2e9 / 4 * 0.85):  # *0.85 just to be save, 4 because float32 is 4 bytes
            # print("process in line 81")
            print(
                    "This output is too large for python process-process communication. "
                    "Saving output temporarily to disk")
            print("output_file {}".format(output_file[:-7] + ".npy"))
            np.save(output_file[:-7] + ".npy", d)
            d = output_file[:-7] + ".npy"
            
            q.put((output_file, (d, dct)))
            # print("process in line 89")
...
def preprocess_multithreaded(trainer, list_of_lists, output_files, num_processes=2, segs_from_prev_stage=None):
    ...
    # print("process in line 101")
    q = Queue(1)
    processes = []
    for i in range(num_processes):
        # print("process in line 104")
        ...
        # print("process in line 110")
        pr.start()
        # print("process in line 112")
        processes.append(pr)

    try:
        # print("process in line 130")
        end_ctr = 0
        while end_ctr != num_processes:
            # print("process in line 133")
            item = q.get()
            if item == "end":
                end_ctr += 1
                continue
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    :return:
    """
    print("num_threads_preprocessing {}".format(num_threads_preprocessing))
    print("num_threads_nifti_save {}".format(num_threads_nifti_save))
    print("output_filenames {} ".format(output_filenames))
...
#         if np.prod(softmax.shape) > (2e9 / bytes_per_voxel * 0.85):  # * 0.85 just to be save
        ...
        # print("get line 298")
        #shutil.copy(output_filename[:-7] + ".npy","/root/Spine/Spine_segmentation/nnUnet_CTSpine1K/data/nnUNet_raw_data_base/nnUNet_raw_data/Task056_VerSe/predict/temp")
        shutil.copy(output_filename[:-7] + ".npy","/root/Spine/Spine/Spine_segmentation/nnUnet_CTSpine1K/data/nnUNet_raw_data_base/nnUNet_raw_data/Task056_VerSe/predict/temp")
        print("output_filename[:-7] {}".format(output_filename[:-7]))
...
#     print(f"result {results}, len {len(results)}")
    _ = [i.get() for i in results]
    # print("get line 272")
    # now apply postprocessing
    # first load the postprocessing properties if they are present. Else raise a well visible warning
    print("disable_postprocessing {}".format(disable_postprocessing))
...
def predict_cases_fast(model, list_of_lists, output_filenames, folds, num_threads_preprocessing,
    ...
    if not disable_postprocessing:
        ...
        if isfile(pp_file):
            ...
            print("line 428 output_filenames[0] {}".format(output_filenames[0]))
            # for_which_classes stores for which of the classes everything but the largest connected component needs to be
            # removed
            for_which_classes, min_valid_obj_size = load_postprocessing(pp_file)
            results.append(pool.starmap_async(load_remove_save,
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
    ...
    print("model in 640 is {}".format(model))
    print("model in 641 is {}".format(output_folder))
